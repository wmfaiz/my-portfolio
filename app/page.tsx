...
const skillGroups = [
  ...
  },
  {
    ...
    items: ["C#", "Python", "JavaScript", "TypeScript", "Go", "Java", "Rust (Basic)"],
  },
  {
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
            ...
            <aside className="space-y-8">
              <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-7 shadow-sm">
                ...
                <div className="mt-5 space-y-4 text-sm">
                  <div>
                    <p className="label">Resume PDF</p>
                    <div className="mt-1 flex flex-col gap-2">
                      <button
                        type="button"
                        onClick={handlePrintCV}
                        className="value cursor-pointer text-left linkish"
                      >
              ...
              <section className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-7 shadow-sm">
                ...
                <div className="mt-5 space-y-5">
                  {education.map((item, index) => (
                    <div key={`education-${index}-${item.institution}-${item.degree}`}>
      ...
      {showCV && (
        <div
          className="cv-print-root fixed inset-0 z-50 flex items-start justify-center bg-black/55 p-2 sm:p-4 md:p-6 print:block print:bg-white print:p-0"
          onClick={() => setShowCV(false)}
        >
          <div
            className="resume-page relative h-[96vh] w-full max-w-[210mm] overflow-y-auto rounded-lg border border-slate-300 bg-white text-slate-900 shadow-2xl print:h-auto print:max-w-none print:overflow-visible print:rounded-none print:border-0 print:shadow-none"
          ...
          >
            <div className="sticky top-0 z-10 flex flex-col gap-3 border-b border-slate-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6 print:hidden">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={handlePrintCV}
                  className="rounded-md bg-slate-900 px-3 py-2 text-sm font-medium text-white transition hover:opacity-90"
                >
                  Print / Save PDF
                </button>
            ...
            <div className="mx-auto max-w-[210mm] px-[12mm] py-[12mm] sm:px-[14mm] sm:py-[14mm] print:max-w-none print:px-0 print:py-0">
              <section className="border-b border-slate-200 pb-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <div className="space-y-1 text-right text-[12px] leading-5 text-slate-700">
                    <p>Malaysia</p>
                    <p>
                      <a href="mailto:wan_faiz@yahoo.com" className="hover:underline">
                        wan_faiz@yahoo.com
                      </a>
                    </p>
                    <p>
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-2 text-[12.5px] leading-[1.6] text-slate-700">
                    ...
                    support cross-platform applications, with additional experience in
                    game systems development and engineering automation.
                  </p>
                </div>
              </section>

              <div className="grid gap-6 pt-4 md:grid-cols-[1.62fr_0.98fr]">
                <div className="space-y-5">
                  <section>
                    <h2 className="border-b border-slate-200 pb-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700">
                      Professional Experience
                    </h2>

                    <div className="mt-3 space-y-4">
                      {experiences.map((job, jobIndex) => (
                        <article key={`cv-exp-${jobIndex}-${job.company}-${job.role}`}>
                          <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <p className="text-[12px] text-slate-500">{job.period}</p>
                          </div>
                          ...
                          <ul className="mt-2 space-y-1">
                            {job.highlights.map((item, itemIndex) => (
                              <li
                                ...
                                className="flex gap-2 text-[12.5px] leading-[1.55] text-slate-700"
                              >
                ...
                <aside className="space-y-5">
                  <section>
                    ...
                    <div className="mt-3 space-y-2 text-[12.5px] leading-[1.55] text-slate-700">
                      <p>
                      ...
                      <p>
                        ...
                        REST APIs, .NET / ASP.NET, OAuth, SSO, System Integration
                      </p>
                      <p>
                        <span className="font-semibold text-slate-900">DevOps:</span>{" "}
                        AWS, Jenkins, Linux, Nginx, Deployment Automation
                      </p>
                      <p>
                        <span className="font-semibold text-slate-900">Frontend:</span>{" "}
                        React, Vue.js, Next.js, jQuery
                      </p>
                      <p>
                        <span className="font-semibold text-slate-900">Databases:</span>{" "}
                    ...
                    <div className="mt-3 space-y-3">
                      {education.map((item, index) => (
                        <div key={`cv-edu-${item.institution}-${index}`}>
                          ...
                          <p className="text-[12.5px] text-slate-700">{item.institution}</p>
                  ...
                  <section>
                    ...
                    <div className="mt-3 space-y-3">
                      {references.map((reference, index) => (
                        <div key={`reference-${index}-${reference.name}`}>
                          <h3 className="text-[12.5px] font-semibold text-slate-900">
                            {reference.name}
                          </h3>
                          <p className="text-[12.5px] text-slate-700">{reference.title}</p>
                          <p className="text-[12.5px] text-slate-700">{reference.company}</p>
